        ...
        >
          ...
          <el-table-column label="照片预览" prop="photoUrl" width="120" align="center">
            <template #default="scope">
              <div :ref="(el) => (imageContainers[scope.$index] = el)" class="image-container">
                <el-image
...
// 初始化图片可见性
const initializePhotoVisibility = () => {
  photosData.value = photosData.value.map((photo) => ({
    ...photo,
...
// 图片加载错误处理
const handleImageError = (row: BuildingPhotosPageVO) => {
  ElMessage.error(`图片加载失败: ${row.photoUrl}`);
  // 可选：设置重试逻辑
  row.isVisible = false; // 标记为不可见，允许重新尝试
};
/** 提交建筑物表单 */
...
function handleQueryPhotos() {
  ...
  BuildingPhotosAPI.getPage(photosQueryParams)
    .then((data) => {
      ...
      // 等待 DOM 更新后设置 IntersectionObserver
      nextTick(() => {
...
const setupIntersectionObserver = () => {
    ...
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          photosData.value[index].isVisible = true;
  ...
  imageContainers.value.forEach((container, index) => {
    container.dataset.index = String(index); // 存储索引
